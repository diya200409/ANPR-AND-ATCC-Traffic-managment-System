...
DB_HOST = 'localhost'
DB_USER = 'root'
DB_PASSWORD = 'Diya@123'
DB_NAME = 'mydb'

import cv2
# Define the license plate cascade
license_plate_cascade = cv2.CascadeClassifier(r"haarcascade_russian_plate_number.xml")
...
def update_database_with_violation(plate_number, host, user, password, database):
    ...
    try:
        ...
        if connection.is_connected():
            ...
            cursor.execute("SELECT violation_count FROM license_plates WHERE plate_number=%s", (plate_number,))
            ...
            if result and result[0] is not None:
                ...
                # Insert new row with violation_count = 1
                cursor.execute(
                    "INSERT INTO license_plates (plate_number, violation_count) VALUES (%s, %s)",
                    (plate_number, 1)
                )
    ...
    try:
        ...
        if connection.is_connected():
            ...
            for record in result:
                plate = record[0]
                violations = record[1] if record[1] is not None else 0  # handle NULL
                print(f"Plate Number: {plate}, Violations: {violations}")
...
def clear_license_plates(host, user, password, database):
    ...
    try:
        ...
        if connection.is_connected():
            cursor = connection.cursor()
            cursor.execute("DELETE FROM license_plates")  # delete all rows
            connection.commit()
            print("All license plate records cleared from database.")
            cursor.close()
...
if __name__ == "__main__":
    main(r"C:\Users\umale\Desktop\traffic_vidotaion.mp4")